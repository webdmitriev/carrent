    ...
    <commentTable :thead="['Name', 'Text', 'Status', 'Change Status', 'Delete']">
      <tbody slot="tbody">
        <tr v-for="comment in comments" :key="comment.id">
          ...
          <td>
            <span @click="chengeComment(comment)" class="link">Change Status</span>
          </td>
          <td>
...
export default {
  ...
  methods: {
    loadComments() {
      axios
        .get("https://minetki-31868.firebaseio.com/comments.json")
        .then(res => {
          let commentsArray = [];
    ...
    chengeComment(comment) {
      ...
      axios.put(
        `https://minetki-31868.firebaseio.com/comments/${comment.id}.json`,
        comment
      );
    },
    deleteComment(id) {
      axios
        .delete(`https://minetki-31868.firebaseio.com/comments/${id}.json`)
        .then(res => {
          this.loadComments();
        });
    }
